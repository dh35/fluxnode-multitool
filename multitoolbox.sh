...
function flux_daemon_bootstrap() {
    ...
    indexb=${richable[$r]}
    BOOTSTRAP_ZIP="http://cdn-2.runonflux.io/apps/fluxshare/getfile/flux_explorer_bootstrap.tar.gz"
    ...
    if [ -f "/home/$USER/$BOOTSTRAP_ZIPFILE" ]; then
	    ...
	    "1)")   
	         ...
	         DB_HIGHT=$(curl -SsL -m 10 http://cdn-2.runonflux.io/apps/fluxshare/getfile/flux_explorer_bootstrap.json | jq -r '.block_height')
		if [[ "$DB_HIGHT" == "" ]]; then
		  DB_HIGHT=$(curl -SsL -m 10 http://cdn-2.runonflux.io/apps/fluxshare/getfile/flux_explorer_bootstrap.json | jq -r '.block_height')
		fi
